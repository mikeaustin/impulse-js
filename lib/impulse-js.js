...
//  if (mainFile) {
    ...
    output.push("var impulse = require('./lib/runtime');");
...
var visitors = {
  ...
  MethodDeclaration: (node, level, parent) => {
    var params = node.params.map(param => generate(param, level, node));
  ...
  BlockStatement: (node, level, parent) => {
    ...
    var methodDeclaration = parent.type === "MethodDeclaration" || parent.type === "ConstructorDeclaration";
